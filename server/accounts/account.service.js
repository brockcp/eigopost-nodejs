...
const emailSend = require('_helpers/email-send');
const emailTemplate = require('_helpers/email-template');
...
async function sendVerificationEmail(account, origin) {
  ...
  if (origin) {
    ...
    message = `<td style="color: #153643;
                          font-family: Arial, sans-serif">
                 <h1 style="text-align:center;
                            font-size: 20px;
                            margin: 0;">
                   Welcome to Eigopost ${account.userName}!
                   <br />
    ...
    message = `<p>
                  Please use the token below to verify your email address
                  with the <code>/account/verify-email</code> api route:
               </p>
               <p><code>${account.verificationToken}</code></p>`;
  }
  await emailSend({
    to: account.email,
    subject: 'Welcome To EigoPost - Please Verify Your Email Address',
...
async function sendPasswordResetEmail(account, origin) {
  ...
  await emailSend({
    to: account.email,
    subject: 'Message From EigoPost - Request To Reset Password',
    html: `${emailTemplate(message)}`
  });
}
